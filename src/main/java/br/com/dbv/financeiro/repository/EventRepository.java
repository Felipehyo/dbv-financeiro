...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    @Query("select new br.com.dbv.financeiro.dto.event.EventDTO(e, COUNT(er.id)) from Event e LEFT JOIN EventRegister er ON e.id = er.event.id WHERE e.club.id = :id AND e.date >= :date GROUP BY e.id")
    List<EventDTO> getEventsAndRegisterByClub(@PathParam("id") Long id, @PathParam("date") LocalDate date);
